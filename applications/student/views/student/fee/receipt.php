...
<head>
	...
	<style type="text/css">
		...
		.school-name {
			font-size: 28px; /* Increase the font size for school name */
			font-family: 'Montserrat-Medium', sans-serif;
			font-weight: bold;
			color: #333;
		}
...
<div class="col-xs-12">
	<div class="col-xs-12 school-name-container">
		<div class="school-logo-container">
			<img src="<?php echo base_url('assets/images/logo/' . $this->config->item('schoolLogo')) ?>" class="school-logo" alt="School Logo" />
		</div>
		<p class="school-name" style="font-size: 20px; font-family: 'Montserrat-Medium', sans-serif; text-align: center;"><?php echo $this->config->item('schoolName') ?></p>
		<p class="school-address" style="font-size: 14px; text-align: center;"><?php echo $this->config->item('schoolAddress') ?></p>
	</div>
		...
		<?php if (isset($reciept)) { ?>
			<?php foreach ($reciept as $row) { ?>
				...
				<tr>
					...
					<tr>
						<th>Paid On</th>
						<td><?php $date = date('d F,Y', strtotime($row->paidondate)); echo $date; ?></td>
					</tr>
					<tr>
						<th>Mode</th>
